...
template <class T> bool Buffer<T>::clear() {
  ...
  if (queue.size() > 0) {
    // Stop adding items to buffer (will return false if an item is currently
    // being added to the buffer)
    if (clearBuffer_add->tryAcquire()) {
      // Stop taking items from buffer (will return false if an item is
      // currently being taken from the buffer)
...
template <class T> int Buffer<T>::size() { return queue.size(); }

template <class T> int Buffer<T>::maxSize() { return bufferSize; }

template <class T> bool Buffer<T>::isFull() {
  return queue.size() == bufferSize;
}

template <class T> bool Buffer<T>::isEmpty() { return queue.size() == 0; }