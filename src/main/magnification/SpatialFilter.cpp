...
void buildLaplacePyrFromImg(const Mat &img, const int levels,
                            vector<Mat> &pyr) {
  pyr.clear();
  Mat currentLevel = img;
...
void buildWaveletPyrFromImg(const Mat &img, const int levels,
                            vector<vector<Mat>> &pyr, int SHRINK_TYPE,
                            float SHRINK_T) {
  float c, dh, dv, dd;
  ...
  for (int lvl = 0; lvl < levels; lvl++) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        // DWT
        c = (curFrame.at<float>(2 * y, 2 * x) +
             curFrame.at<float>(2 * y, 2 * x + 1) +
             curFrame.at<float>(2 * y + 1, 2 * x) +
             curFrame.at<float>(2 * y + 1, 2 * x + 1)) *
            0.5;
        dh = (curFrame.at<float>(2 * y, 2 * x) +
              curFrame.at<float>(2 * y + 1, 2 * x) -
              curFrame.at<float>(2 * y, 2 * x + 1) -
              curFrame.at<float>(2 * y + 1, 2 * x + 1)) *
             0.5;
        dv = (curFrame.at<float>(2 * y, 2 * x) +
              curFrame.at<float>(2 * y, 2 * x + 1) -
              curFrame.at<float>(2 * y + 1, 2 * x) -
              curFrame.at<float>(2 * y + 1, 2 * x + 1)) *
             0.5;
        dd = (curFrame.at<float>(2 * y, 2 * x) -
              curFrame.at<float>(2 * y, 2 * x + 1) -
              curFrame.at<float>(2 * y + 1, 2 * x) +
              curFrame.at<float>(2 * y + 1, 2 * x + 1)) *
             0.5;
...
void buildImgFromLaplacePyr(const vector<Mat> &pyr, const int levels,
                            Mat &dst) {
...
void buildImgFromWaveletPyr(const vector<vector<Mat>> &pyr, Mat &dst,
                            Size origSize, int SHRINK_TYPE, float SHRINK_T) {
  int levels = pyr.size();
  float c, dh, dv, dd;
  Mat tmp;
  int width, height;
