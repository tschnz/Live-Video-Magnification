...
void RieszPyramidLevel::build(const cv::Mat &octave) {
  ...
  itsLp = octave;
  cv::filter2D(itsLp, real(itsR), itsLp.depth(), realK, cv::Point(-1, -1), 0,
               cv::BORDER_REFLECT_101);
  cv::filter2D(itsLp, imag(itsR), itsLp.depth(), imagK, cv::Point(-1, -1), 0,
               cv::BORDER_REFLECT_101);
}
...
void RieszPyramidLevel::unwrapOrientPhase(const RieszPyramidLevel &prior) {
  cv::Mat temp1 = itsLp.mul(prior.itsLp) + real(itsR).mul(real(prior.itsR)) +
                  imag(itsR).mul(imag(prior.itsR));
  cv::Mat temp2 = real(itsR).mul(prior.itsLp) - real(prior.itsR).mul(itsLp);
  cv::Mat temp3 = imag(itsR).mul(prior.itsLp) - imag(prior.itsR).mul(itsLp);
...
void RieszPyramidLevel::normalize(CompExpMat &result) {
  ...
  static const cv::Mat kernel = cv::getGaussianKernel(aperture, sigma, CV_32F);
  ...
  cv::sepFilter2D(sin(result), sin(result), -1, kernel, kernel,
                  cv::Point(-1, -1), 0, cv::BORDER_REFLECT_101);
  cv::Mat temp;
  cv::sepFilter2D(amplitude, temp, -1, kernel, kernel, cv::Point(-1, -1), 0,
                  cv::BORDER_REFLECT_101);
...
void RieszPyramid::buildPyramid(const cv::Mat &frame) {
  ...
  for (int i = 0; i < max; ++i) {
    ...
    cv::filter2D(octave, hp, CV_32F, highPassFilter, cv::Point(-1, -1), 0,
                 cv::BORDER_REFLECT_101);
    pyrLevels[i].build(hp);

    // Lowpass is passed onto the next level
    cv::filter2D(octave, lp, CV_32F, 2.0 * lowPassFilter, cv::Point(-1, -1), 0,
                 cv::BORDER_REFLECT_101);
    octave = subsample(lp);
  }
...
const cv::Mat RieszPyramid::collapsePyramid() {
  ...
  for (int i = count - 1; i >= 0; --i) {
    ...
    cv::filter2D(up_zero, lp, CV_32F, 2.0 * lowPassFilter, cv::Point(-1, -1), 0,
                 cv::BORDER_REFLECT_101);
