...
ProcessingThread::ProcessingThread(SharedImageBuffer *sharedImageBuffer,
                                   int deviceNumber)
    : QThread(), sharedImageBuffer(sharedImageBuffer), emitOriginal(false) {
  // Save Device Number
...
void ProcessingThread::run() {
  ...
  while (1) {
    ...
    currentFrame =
        Mat(sharedImageBuffer->getByDeviceNumber(deviceNumber)->get().clone(),
            currentROI);
    ...
    // Save the original Frame after grayscale conversion, so VideoWriter works
    // correct
...
void ProcessingThread::getOriginalFrame(bool doEmit) { emitOriginal = doEmit; }

int ProcessingThread::getRecordFPS() { return recordingFramerate; }
...
void ProcessingThread::updateImageProcessingFlags(
...
void ProcessingThread::updateImageProcessingSettings(
...
bool ProcessingThread::startRecord(std::string filepath, bool captureOriginal) {
  ...
  bool isColor =
      !((imgProcFlags.grayscaleOn) || (currentFrame.channels() == 1));
...
bool ProcessingThread::isRecording() { return this->doRecord; }
