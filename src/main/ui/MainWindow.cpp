...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(connectToCameraButton, SIGNAL(released()), this,
          SLOT(connectToCamera()));
  ...
  connect(ui->actionAboutQt, SIGNAL(triggered()), this,
          SLOT(showAboutQtDialog()));
  connect(ui->actionQuit, SIGNAL(triggered()), this, SLOT(close()));
  connect(ui->actionFullScreen, SIGNAL(toggled(bool)), this,
          SLOT(setFullScreen(bool)));
  connect(ui->actionConnect_Open, SIGNAL(triggered()), this,
          SLOT(connectToCamera()));
...
void MainWindow::connectToCamera() {
  // Get next tab index
  int nextTabIndex = (deviceNumberMap.size() + fileNumberMap.size() == 0)
                         ? 0
                         : ui->tabWidget->count();
  // Show dialog
  ...
  if (cameraConnectDialog->exec() == QDialog::Accepted) {
    if (cameraConnectDialog->isCamera()) {
      ...
      if (!deviceNumberMap.contains(deviceNumber)) {
        ...
        Buffer<Mat> *imageBuffer =
            new Buffer<Mat>(cameraConnectDialog->getImageBufferSize());
        // Add created ImageBuffer to SharedImageBuffer object
        sharedImageBuffer->add(deviceNumber, imageBuffer);
        // Create CameraView
        cameraViewMap[deviceNumber] =
            new CameraView(ui->tabWidget, deviceNumber, sharedImageBuffer);
        // Attempt to connect to camera
        if (cameraViewMap[deviceNumber]->connectToCamera(
          ...
          // Add tab
          ui->tabWidget->addTab(cameraViewMap[deviceNumber],
                                tabLabel + " [" +
    ...
    // Attempt to load a file
      ...
      if (filepath.isEmpty()) {
        QMessageBox::warning(this, tr("ERROR:"),
                             tr("Please enter path to video file."));
        connectToCamera();
        return;
      }
      // Load the File
      ...
      if (file.exists()) {
        ...
        if (videoViewMap[filename]->loadVideo(
                cameraConnectDialog->getPlayerThreadPrio(),
                cameraConnectDialog->getResolutionWidth(),
                cameraConnectDialog->getResolutionHeight(),
                cameraConnectDialog->getFpsNumber())) {
          // Add to map
          ...
          if (nextTabIndex == 0)
            ui->tabWidget->removeTab(0);
          // Add tab
          ui->tabWidget->addTab(videoViewMap[filename],
                                tabLabel + "[" + filename + "]");
        ...
        // Could not load video
          ...
          QMessageBox::warning(
              this, tr("ERROR:"),
              tr("Could not convert Video. Please check file format."));
          // Explicitly delete widget
...
void MainWindow::showAboutDialog() {
  ...
  ts << "<h3>Realtime Video Magnification</h3>"
     << "<p>This is an OpenCV and Qt based realtime application to magnify "
        "motion and color in videos and camerastreams.</p>"
     << "<p>Copyright &copy; 2015 Jens Schindel</p>"
     << "<p>The provided code is licensed under GPLv3 and hosted at "
     << "<a "
        "href='https://github.com/tschnz/Realtime-Video-Magnification'>GitHub</"
        "a>.</p>"
     << "<p>This application is based on the work of <a "
        "href='https://github.com/wzpan/QtEVM'>Joseph Pan</a> and "
     << "<a href='https://github.com/nickdademo/qt-opencv-multithreaded'>Nick "
        "D&#39;Ademo</a>.</p>"
     << "<p>The underlying algorithm is part of a research project originated "
        "at the MIT, called "
     << "<a href='http://people.csail.mit.edu/mrub/vidmag/'>Eulerian Video "
        "Magnification</a></p>";
...
void MainWindow::showHelpDialog() {
  ...
  ts << "<p>The best way to understand is experimenting with the sliders. "
        "Additionally there are tooltips for every option, "
     << "appearing while hovering over the labels/texts to the left of every "
        "slider/spinbox/checkbox.</p><p>If you should encounter "
     << "crashs while color magnifying, try a lower resolution or change ROI "
        "(draw a box in the frame label).</p>"
     << "<p>If you want to understand how this algorithm works please visit "
        "the MITs "
     << "<a href='http://people.csail.mit.edu/mrub/vidmag/'>Eulerian Video "
        "Magnification</a> research page.</p>";
...
bool MainWindow::removeFromMapByTabIndex(QMap<QString, int> &map,
                                         int tabIndex) {
...
void MainWindow::setTabCloseToolTips(QTabWidget *tabs, QString tooltip) {
  QList<QAbstractButton *> allPButtons =
      tabs->findChildren<QAbstractButton *>();
...
void MainWindow::addCodecs() {
  ...
  connect(codecMenu, SIGNAL(triggered(QAction *)), this,
          SLOT(setCodec(QAction *)));
