...
MagnifyOptions::MagnifyOptions(QWidget *parent)
    : QWidget(parent), ui(new Ui::MagnifyOptions) {
  ...
  connect(ui->MagnifcationtypeComboBox, SIGNAL(currentIndexChanged(int)),
          SLOT(updateFlagsFromOptionsTab()));
  connect(ui->MagnifcationtypeComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(reset(int)));

  // Update Settings
  connect(ui->COLowDoubleSpinBox, SIGNAL(valueChanged(double)),
          SLOT(updateSettingsFromOptionsTab()));
  connect(ui->COHighDoubleSpinBox, SIGNAL(valueChanged(double)),
          SLOT(updateSettingsFromOptionsTab()));
  connect(ui->ChromSpinBox, SIGNAL(valueChanged(int)),
          SLOT(updateSettingsFromOptionsTab()));
  connect(ui->AmplificationSpinBox, SIGNAL(valueChanged(int)),
          SLOT(updateSettingsFromOptionsTab()));
  connect(ui->COWavelengthSpinBox, SIGNAL(valueChanged(double)),
          SLOT(updateSettingsFromOptionsTab()));
  connect(ui->LevelsSpinBox, SIGNAL(valueChanged(int)),
          SLOT(updateSettingsFromOptionsTab()));

  // Update Spinbox
  connect(ui->COWavelengthSlider, SIGNAL(valueChanged(int)), this,
          SLOT(convertFromSlider(int)));
  connect(doubleSlider, SIGNAL(lowerPositionChanged(int)), this,
          SLOT(convertFromSlider(int)));
  connect(doubleSlider, SIGNAL(upperPositionChanged(int)), this,
          SLOT(convertFromSlider(int)));
  connect(ui->ChromSlider, SIGNAL(valueChanged(int)), this,
          SLOT(convertFromSlider(int)));

  // Update Slider
  connect(ui->COWavelengthSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(convertFromSpinBox(double)));
  connect(ui->COLowDoubleSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(convertFromSpinBox(double)));
  connect(ui->COHighDoubleSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(convertFromSpinBox(double)));

  // Other
  connect(ui->resetButton, SIGNAL(clicked()), SLOT(reset()));
  connect(ui->grayscaleCheckBox, SIGNAL(clicked()),
          SLOT(updateFlagsFromOptionsTab()));
...
MagnifyOptions::~MagnifyOptions() { delete ui; }
...
// Reset everything to Default, configured in Config.h, depending on Magnify
// Type. On Start its 0
void MagnifyOptions::reset() {
  ...
  default:
    ...
    ui->verticalSpacer->changeSize(0, 0, QSizePolicy::Maximum,
                                   QSizePolicy::Maximum);
...
void MagnifyOptions::reset(int type) { reset(); }
...
void MagnifyOptions::updateFlagsFromOptionsTab() {
  ...
  imgProcFlags.colorMagnifyOn =
      (ui->MagnifcationtypeComboBox->currentIndex() == 1);
  imgProcFlags.laplaceMagnifyOn =
      (ui->MagnifcationtypeComboBox->currentIndex() == 2);
  imgProcFlags.rieszMagnifyOn =
      (ui->MagnifcationtypeComboBox->currentIndex() == 3);
...
ImageProcessingFlags MagnifyOptions::getFlags() { return this->imgProcFlags; }

void MagnifyOptions::applyColorInterface() {
  ui->LevelsSpinBox->setDisabled(false);
  ui->verticalSpacer->changeSize(0, 20, QSizePolicy::Maximum,
                                 QSizePolicy::Maximum);
...
void MagnifyOptions::applyRieszInterface() {
  ...
  double maxFr = 100.0;
  doubleSlider->setMaximum(maxFr * 100);
  ui->COHighDoubleSpinBox->setMaximum(maxFr);
  ui->COLowDoubleSpinBox->setMaximum(maxFr);
  ui->DoubleSliderLabel->show();
  ui->COHighDoubleSpinBox->show();
  ui->COLowDoubleSpinBox->show();
  doubleSlider->show();
