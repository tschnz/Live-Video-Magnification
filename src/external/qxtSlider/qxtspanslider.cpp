...
QxtSpanSliderPrivate::QxtSpanSliderPrivate()
    : lower(0), upper(0), lowerPos(0), upperPos(0), offset(0), position(0),
      lastPressed(NoHandle), mainControl(LowerHandle),
      lowerPressed(QStyle::SC_None), upperPressed(QStyle::SC_None),
      movement(QxtSpanSlider::FreeMovement), firstMovement(false),
      blockTracking(false) {}

// TODO: get rid of this in Qt 4.3
void QxtSpanSliderPrivate::initStyleOption(QStyleOptionSlider *option,
                                           SpanHandle handle) const {
  if (!option)
    return;
  ...
  option->upsideDown =
      (p->orientation() == Qt::Horizontal)
          ? (p->invertedAppearance() != (option->direction == Qt::RightToLeft))
          : (!p->invertedAppearance());
...
int QxtSpanSliderPrivate::pixelPosToRangeValue(int pos) const {
  ...
  int sliderLength = 0;
  const QSlider *p = &qxt_p();
  const QRect gr = p->style()->subControlRect(QStyle::CC_Slider, &opt,
                                              QStyle::SC_SliderGroove, p);
  const QRect sr = p->style()->subControlRect(QStyle::CC_Slider, &opt,
                                              QStyle::SC_SliderHandle, p);
  if (p->orientation() == Qt::Horizontal) {
  ...
  return QStyle::sliderValueFromPosition(p->minimum(), p->maximum(),
...
void QxtSpanSliderPrivate::setupPainter(QPainter *painter,
                                        Qt::Orientation orientation, qreal x1,
...
void QxtSpanSliderPrivate::drawSpan(QStylePainter *painter,
  ...
  // area
  QRect groove = p->style()->subControlRect(QStyle::CC_Slider, &opt,
                                            QStyle::SC_SliderGroove, p);
...
/*!
    ...
    \value NoCrossing The handles cannot cross, but they can still overlap each
   other. The lower and upper values can be the same. \value NoOverlapping The
   handles cannot overlap each other. The lower and upper values cannot be the
   same.
 */
...
QxtSpanSlider::QxtSpanSlider(QWidget *parent) : QSlider(parent) {
  ...
  connect(this, SIGNAL(rangeChanged(int, int)), &qxt_d(),
...
void QxtSpanSlider::setHandleMovementMode(
    QxtSpanSlider::HandleMovementMode mode) {
  qxt_d().movement = mode;
}
...
void QxtSpanSlider::setLowerValue(int lower) { setSpan(lower, qxt_d().upper); }
...
void QxtSpanSlider::setUpperValue(int upper) { setSpan(qxt_d().lower, upper); }
 ...
 */
int QxtSpanSlider::lowerPosition() const { return qxt_d().lowerPos; }
 ...
 */
int QxtSpanSlider::upperPosition() const { return qxt_d().upperPos; }
...
void QxtSpanSlider::mousePressEvent(QMouseEvent *event) {
  ...
  qxt_d().handleMousePress(event->pos(), qxt_d().upperPressed, qxt_d().upper,
                           QxtSpanSliderPrivate::UpperHandle);
  if (qxt_d().upperPressed != QStyle::SC_SliderHandle)
    qxt_d().handleMousePress(event->pos(), qxt_d().lowerPressed, qxt_d().lower,
...
void QxtSpanSlider::mouseMoveEvent(QMouseEvent *event) {
  ...
  int newPosition =
      qxt_d().pixelPosToRangeValue(qxt_d().pick(event->pos()) - qxt_d().offset);
  if (m >= 0) {
